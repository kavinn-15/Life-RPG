...
export async function request(path, options = {}) {
  ...
  const exec = async () => {
    ...
    try {
      ...
      // If 401 unauthorized on a non-auth path, try to refresh demo login once
      if (res.status === 401 && !path.startsWith('/auth/')) {
        ...
        if (newToken) {
          ...
          const data = retryJson.data !== undefined ? retryJson.data : retryJson;
          if (isGet) {
            responseCache.set(cacheKey, { data, expiresAt: Date.now() + 15000 });
          } else {
