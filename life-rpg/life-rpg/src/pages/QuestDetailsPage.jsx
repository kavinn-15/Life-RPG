...
export default function QuestDetailsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    Promise.all([questService.getQuestById(id), questService.getActiveQuests(), domainService.getDomains()]).then(
      ([foundQuest, activeQuests, domains]) => {
        if (cancelled) return;
        setQuest(foundQuest);
        setIsActive(activeQuests.some((q) => q.id === id));
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      grantRewards({ xp: result.xp, gold: result.gold, statKey: result.statKey, statAmount: result.statAmount });
      pushToast(`${quest.title} complete · +${result.xp} XP`);
      navigate('/quests');
    } catch {
      pushToast("Can't complete a quest that isn't in your active queue yet", 'info');
    } finally {
      setBusy(false);
    }
  };
