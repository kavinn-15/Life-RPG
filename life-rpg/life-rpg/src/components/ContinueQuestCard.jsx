...
export default function ContinueQuestCard({ quest }) {
  ...
  const handleComplete = () => {
    ...
    setTimeout(() => {
      setStatus('done');
      grantRewards({ xp: quest.xp, gold: quest.gold, statKey: quest.statKey, statAmount: 3 });
    ...
    <div className="bg-surface-container-lowest rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between group">
      <div className="flex gap-4">
        <div className="w-16 h-16 rounded-xl shrink-0 bg-surface-container-high flex items-center justify-center text-primary">
          <span className="material-symbols-outlined fill text-3xl">{quest.icon}</span>
        </div>
        <div className="flex flex-col min-w-0 flex-1">
          <div className="flex items-center justify-between gap-1 mb-1">
            <span className={`px-2.5 py-0.5 rounded-full font-label-caps text-label-caps ${quest.domainClass}`}>
              {quest.domain}
            </span>
      ...
      <div className="mt-4 pt-3 flex items-center justify-between gap-3">
        <div className="flex-1 flex flex-col gap-1">
          <div className="w-full h-2 bg-surface-container rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full ${quest.progressClass}`}
              style={{ width: `${status === 'done' ? 100 : quest.progress}%` }}
            />
          </div>
          <span className="font-label-caps text-label-caps text-outline">
            {status === 'done' ? 'Complete!' : quest.progressLabel}
          </span>
        </div>
        <button
          className={[
