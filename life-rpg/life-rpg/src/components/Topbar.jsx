      ...
      <header className="fixed top-0 lg:left-[260px] left-0 right-0 h-20 bg-surface/85 backdrop-blur-xl z-40 shadow-[0_1px_8px_rgba(20,19,43,0.04)]">
        <div className="h-20 w-full px-3 sm:px-6 flex items-center justify-between gap-2.5 lg:gap-4">
          ...
          <div className="flex items-center gap-3 lg:gap-4 min-w-0 flex-1">
            {/* Mobile brand mark */}
            <Link to="/" className="lg:hidden flex items-center gap-2 shrink-0 group">
              <div className="w-9 h-9 rounded-xl bg-ink-card border border-primary/40 flex items-center justify-center p-0.5 shadow-[0_0_10px_rgba(110,86,248,0.3)] overflow-hidden">
                <img
                  src="/logo.png"
                  alt="Life RPG Logo"
                  className="w-full h-full object-cover rounded-lg transform scale-110"
                />
              </div>
            ...
            <div className="hidden md:flex flex-col shrink-0">
              ...
              <div className="flex items-center gap-1.5 font-headline-sm text-headline-sm text-on-surface font-extrabold whitespace-nowrap">
                <span>Welcome back,</span>
                <span className="text-primary font-extrabold">👋 {state.playerName}</span>
              </div>
            </div>

            {/* Search Bar (flexes cleanly without overflowing) */}
            <form onSubmit={handleSearchSubmit} className="relative items-center hidden lg:flex flex-1 min-w-[140px] max-w-[240px]">
          ...
          <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
            ...
            {/* Character Avatar with Quick Upload & Sheet Access */}
            <div className="relative flex items-center gap-1 shrink-0">
              <button
                type="button"
                onClick={() => setAvatarModalOpen(true)}
                className="relative cursor-pointer group shrink-0 focus:outline-none"
                title="Change Avatar Photo"
                aria-label="Change Avatar Photo"
              >
                <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center shadow-[0_2px_0_#4029ba] group-hover:scale-105 transition-transform overflow-hidden ring-2 ring-surface-container-lowest">
                  {state.avatarUrl ? (
                    <img
                      src={state.avatarUrl}
